...
res.status = function status(code) {
  ...
  if (typeof code !== 'number' || !Number.isInteger(code)) {
    throw new TypeError(`Expected an integer HTTP status code, but received: ${JSON.stringify(code)} (${typeof code})`);
  }
  // Check if the status code is outside of Node's valid range
  if (code < 100 || code > 999) {
    throw new RangeError(`Invalid status code: ${JSON.stringify(code)}. Status code must be between 99 and 999.`);
  }
